...
- (void)setup {
    ...
    _tokenField.font = [UIFont systemFontOfSize:13.0f];
	...
	[_separator setBackgroundColor:[UIColor clearColor]];
	...
	else
	{
		...
		[_resultsTable setSeparatorColor:[UIColor clearColor]];
...
- (void)tokenFieldFrameDidChange:(TITokenField *)field {
    NSLog(@"%f",field.frame.size.height);
	[self updateContentSize];
}
...
- (void)didEndEditing {
	...
	if (_removesTokensOnEndEditing){
		...
		if (_tokens.count){
			...
			CGSize untokSize = [untokenized sizeWithFont:[UIFont systemFontOfSize:13]];
...
+ (UIColor *)blueTintColor {
    return [UIColor lightGrayColor];//[UIColor colorWithRed:0.216 green:0.373 blue:0.965 alpha:1];
}
