...
#import "SQSegment.h"
#import "SQSegmentIndicator.h"

@interface SQSegmentedControl ()

@property NSArray *segments;
@property SQSegmentIndicator *selectedSegmentIndicator;

- (void)moveSelectedSegmentIndicatorToSegmentAtIndex:(NSUInteger)index animated:(BOOL)animated;
- (CGRect)indicatorFrameForSegment:(SQSegment *)segment;

@end

@implementation SQSegmentedControl
...
- (instancetype)initWithItems:(NSArray *)items {
    ...
    if (self) {
        ...
        for (NSString *segmentTitle in items) {
            SQSegment *segment = [[SQSegment alloc] initWithTitle:segmentTitle];
...
- (void)initialize {
    ...
    self.selectedSegmentIndicator = [[SQSegmentIndicator alloc] initWithFrame:CGRectZero];
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    for (SQSegment *segment in self.segments) {
...
- (void)layoutSubviews {
    ...
    for (int i = 0; i < [self.segments count]; i++) {
        SQSegment *segment = self.segments[i];
...
- (void)insertSegmentWithTitle:(NSString *)title atIndex:(NSUInteger)index {
    ...
    SQSegment *newSegment = [[SQSegment alloc] initWithTitle:title];
...
- (void)removeSegmentAtIndex:(NSUInteger)index {
    ...
    SQSegment *segment = self.segments[index];
...
- (void)moveSelectedSegmentIndicatorToSegmentAtIndex:(NSUInteger)index animated:(BOOL)animated {
    if (index != self.selectedSegmentIndex) {
        SQSegment *previousSegment = self.segments[self.selectedSegmentIndex];
        previousSegment.titleLabel.font = self.titleFont;
        previousSegment.titleLabel.textColor = self.titleTextColor;
    }
    
    SQSegment *selectedSegment = self.segments[index];
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    if (CGRectContainsPoint(self.selectedSegmentIndicator.bounds, [touch locationInView:self.selectedSegmentIndicator])) {
        ...
        [self.segments enumerateObjectsUsingBlock:^(SQSegment *segment, NSUInteger index, BOOL *stop) {
